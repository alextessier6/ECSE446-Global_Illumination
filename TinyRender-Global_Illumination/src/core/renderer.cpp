...
void Renderer::render() {
    if (realTime) {
        ...
        v4f origin(scene.config.camera.o, 1);
        ...
        for (int i = 0; i < omp_get_max_threads(); ++i) {
            samplers.emplace_back(260742984 + i);
        }
            ...
            for (int y = 0; y < scene.config.height; y++) {
                ...
                if(scene.config.spp > 1){
                    for (int i = 0; i < scene.config.spp; i++) {
                        ...
                        //Retrieve pixel  (x+random, y+random) over [-1, 1]
                        float xBox = ((2 * (x + sampler.next()) / scene.config.width) - 1);
                        float yBox = (1 - (2 * (y + sampler.next()) / scene.config.height));

                        //Scale, taking into account the fov and the aspectRatio
                        float xScaled = xBox * scaling * aspectRatio;
                        float yScaled = yBox * scaling;
                        float zScaled = -1.f;

                        //Direction is pt on image plane since the camera is at the origin
                        v4f dir(xScaled, yScaled, zScaled, 1.f);

                        //Converts to camera view and normalize
                        v4f dirWorld = dir * inverseView;
                        dirWorld = normalize(dirWorld);

                        //Creates ray and integrates
                        Ray ray{origin, dirWorld};
                        color += integrator->render(ray, sampler);

                    }
                }
                else {
                    //For Veach when spp = 1
                    float xBox = ((2 * (x + 0.5) / scene.config.width) - 1);
                    float yBox = (1 - (2 * (y + 0.5) / scene.config.height));
...
//            }
                //Outputs the average sampled color to the pixel buffer
                integrator->rgb->data[(y * scene.config.width) + x] = color / scene.config.spp;
            }
        }
    }
}
